...
import { dayRoutes } from "@/data/iceland";

/**
 * Overview map for the Iceland plan: one colored polyline per day, toggled by
 * chip buttons — the same idiom as the Sicily routes map.
 */
export default function IcelandRoutes() {
  ...
  const [visible, setVisible] = useState<Record<string, boolean>>(
    Object.fromEntries(dayRoutes.map((r) => [r.id, true]))
  );
      ...
      for (const route of dayRoutes) {
        ...
        route.stops.forEach((s, i) => {
          L.circleMarker([s.lat, s.lng], {
            ...
            .bindPopup(`<b>${s.name}</b><br/>${route.name}${s.note ? ` · ${s.note}` : ""}`)
            .addTo(group);
        });
  ...
  useEffect(() => {
    ...
    const key = dayRoutes.map((r) => (visible[r.id] ? "1" : "0")).join("");
    if (key !== prevKeyRef.current) {
      prevKeyRef.current = key;
      const pts: [number, number][] = dayRoutes
